...
# <Scene scene {'id': 's_bluemax6', 'xsi:schemaLocation': 'http://datenighto.com/schemas/htmlvn/0.1 ./htmlvn.xsd', 'xmlns': 'http://datenighto.com/schemas/htmlvn/0.1', 'xmlns:xsi': 'http://www.w3.org/2001/XMLSchema-instance', 'kind': 'scene'} ''>
label s_bluemax6:
    scene i_sw_black with dissolve
    stop music fadeout 1.0
    $ slot_day_name = "s_bluemax6"
    $ renpy.pause(1) # buffer
    # <Scene scene {'id': 's_bluemax6', 'xsi:schemaLocation': 'http://datenighto.com/schemas/htmlvn/0.1 ./htmlvn.xsd', 'xmlns': 'http://datenighto.com/schemas/htmlvn/0.1', 'xmlns:xsi': 'http://www.w3.org/2001/XMLSchema-instance', 'kind': 'scene'} ''>
    # Pass (assets)
    # <Events events {'kind': 'events'} ''>
    # <ParallelEvent ParallelEvent {'auto': 'true', 'name': 'initialize', 'kind': 'ParallelEvent'} ''>
    label s_bluemax6_initialize:
        # <ParallelEvent ParallelEvent {'auto': 'true', 'name': 'initialize', 'kind': 'ParallelEvent'} ''>
        # <Events events {'kind': 'events'} ''>
        # Pass (VarEnsure)
        ...
        show i_battlepose_robotarmy as robotarmy zorder 12:
            ...
            xpos 0.1 
            alpha 0.0
        show i_battlepose_bluemax as cousin zorder 13:
            default
            xpos 0.9 
            ypos 0.5 
            alpha 0.0
            xzoom 0.8 
            yzoom 0.8 
        show i_battlepose_bluemax as bluemax zorder 12:
            default
            xpos 0.9 
            ypos 0.38 
            xzoom 0.75 
    ...
    show i_sw_white as white_swatch    # ActorEvent
    with flash
    show i_battlepose_bluemax as cousin:
        # ActorEvent
        # ActorEvent (Transition only)
        linear 0.75 alpha 1.0
    $ renpy.pause(0.75, hard=True) # TimedPause
    ...
    # End battlemacro
    if slot_pick_count_bluemax <= 1:
        call BadEndMacro
    # <NormalEndMacro NormalEndMacro {'cousinX': '-5%', 'name': '_3', 'partnerActor': 'bluemax', 'partnerImage': '@i_bluemax_stand_smile', 'partnerX': '-35%', 'kind': 'NormalEndMacro'} ''>
    # <ParallelEvent ParallelEvent {'auto': True, 'kind': 'ParallelEvent'} ''>
    # <Events events {'kind': 'events'} ''>
    ...
    call CreditsEvent
    if slot_pick_count_bluemax >= 3:
        # <IfGTE IfGTE {'auto': 'true', 'lvalue': '${slot:pick_count_bluemax}', 'name': '_6', 'rvalue': '3', 'kind': 'IfGTE'} ''>
        label s_bluemax6__6:
            # <IfGTE IfGTE {'auto': 'true', 'lvalue': '${slot:pick_count_bluemax}', 'name': '_6', 'rvalue': '3', 'kind': 'IfGTE'} ''>
            # <Events events {'kind': 'events'} ''>
            $ persistent.got_trueend_bluemax = True
            show i_event_bluemax_ending as trueEnd zorder 23:
                default
                alpha 0.0
                linear 1 alpha 1.0
                # ShowWithAtl
