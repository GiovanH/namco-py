...
# <Scene scene {'id': 's_amazona6', 'xsi:schemaLocation': 'http://datenighto.com/schemas/htmlvn/0.1 ./htmlvn.xsd', 'xmlns': 'http://datenighto.com/schemas/htmlvn/0.1', 'xmlns:xsi': 'http://www.w3.org/2001/XMLSchema-instance', 'kind': 'scene'} ''>
label s_amazona6:
    scene i_sw_black with dissolve
    stop music fadeout 1.0
    $ slot_day_name = "s_amazona6"
    $ renpy.pause(1) # buffer
    # <Scene scene {'id': 's_amazona6', 'xsi:schemaLocation': 'http://datenighto.com/schemas/htmlvn/0.1 ./htmlvn.xsd', 'xmlns': 'http://datenighto.com/schemas/htmlvn/0.1', 'xmlns:xsi': 'http://www.w3.org/2001/XMLSchema-instance', 'kind': 'scene'} ''>
    # Pass (assets)
    # <Events events {'kind': 'events'} ''>
    # <ParallelEvent ParallelEvent {'auto': 'true', 'name': 'initialize', 'kind': 'ParallelEvent'} ''>
    label s_amazona6_initialize:
        # <ParallelEvent ParallelEvent {'auto': 'true', 'name': 'initialize', 'kind': 'ParallelEvent'} ''>
        # <Events events {'kind': 'events'} ''>
        # Pass (VarEnsure)
        ...
        show i_battlepose_cousin_normal as cousin zorder 13:
            ...
            xzoom 0.75 
            yzoom 0.75 
        show i_battlepose_amazona as amazona zorder 12:
            default
            xpos 0.9 
            ypos 0.38 
            xzoom 0.75 
    ...
    # <BattleMacro BattleMacro {'name': '_1', 'partnerActor': 'amazona', 'kind': 'BattleMacro'} ''>
    window hide None
    ...
    show i_sw_white as white_swatch    # ActorEvent
    with flash
    show i_battlepose_amazona as amazona:
        # ActorEvent
        # ActorEvent (Transition only)
        linear 0.75 alpha 1.0
    $ renpy.pause(0.75, hard=True) # TimedPause
    ...
    show i_amazona_akimbo_smile as amazona zorder 2:
        # ActorEvent
        xzoom 1.0 
        yzoom 1.0 
        xpos 0.7 
    ...
    # End battlemacro
    if slot_pick_count_amazona <= 1:
        call BadEndMacro
    # <NormalEndMacro NormalEndMacro {'cousinX': '-15%', 'name': '_3', 'partnerActor': 'amazona', 'partnerImage': '@i_amazona_akimbo_smile', 'partnerX': '-35%', 'kind': 'NormalEndMacro'} ''>
    # <ParallelEvent ParallelEvent {'auto': True, 'kind': 'ParallelEvent'} ''>
    # <Events events {'kind': 'events'} ''>
    ...
    call CreditsEvent
    if slot_pick_count_amazona >= 3:
        # <IfGTE IfGTE {'auto': 'true', 'lvalue': '${slot:pick_count_amazona}', 'name': '_6', 'rvalue': '3', 'kind': 'IfGTE'} ''>
        label s_amazona6__6:
            # <IfGTE IfGTE {'auto': 'true', 'lvalue': '${slot:pick_count_amazona}', 'name': '_6', 'rvalue': '3', 'kind': 'IfGTE'} ''>
            # <Events events {'kind': 'events'} ''>
            $ persistent.got_trueend_amazona = True
            show i_event_amazona_ending as trueEnd zorder 23:
                default
                alpha 0.0
                linear 1 alpha 1.0
                # ShowWithAtl
